...
acute_mi_cl <- importCodelist(
  path = here("Cohorts", "conditions", "acute_mi.csv"),
  type = "csv"
)
...
cdm$mi_inpatient_first <- cdm$acute_mi |>
  requireInDateRange(
...
stroke_cl <- importCodelist(
  path = here("Cohorts", "conditions", "stroke_broad.csv"),
  type = "csv"
)
...
cdm$stroke <- conceptCohort(
  ...
  collapseCohorts(gap = 28, name = "stroke")

cdm$stroke_inpatient_first <- cdm$stroke |>
  requireInDateRange(
    dateRange = study_period,
...
af_cl <- importCodelist(
  path = here("Cohorts", "conditions", "atrial_fibrillation.csv"),
  type = "csv"
)
...
ckd_diag_codes <- list(ckd = c(
  765535, 46284566, 46284567, 46284570, 443614, 46270354, 601161, 44782703, 
  45773576, 43531559, 44792226, 44792227, 44784640, 43021853, 762000, 46284572,
  46287169,46284575, 443601,	46270355,	601162,	44782692,	45769901,	43531566,
  44792228,	44792229,	45757447,	43021836,	43021854, 37019193,	762001,	46284587,	
  46286992, 46284588,	46284591,	46284592,	46284593,	443597,	46273636, 601163,
  44782691,	45771075,	43531653, 44792230,	44792231,	45763854, 44792232,	
  44792249,	45763855,	44792250, 44792251,	45757446,	43021835,	43020456,	762033, 
  765536,	46284597,46284598,	46284599, 443612,	46273514,	601164,	44782689,
  45769902,	43531577,	44792252,	44792253, 45757445,	44784639,	43020457,
  762034, 45768813,	760850,	46284600,	46284602, 46284603,	443611, 46270356,
  601165,	44782690,	45769903,	43531562, 37017813,	44792254,	37018761,	44792255,
  46273164,	37018886,	601166,	44782717, 45769904,	45769906,	4030520,	4128200,
  4125970, 193782, 45772751, 45757393,	45757392,	45757444,	762973,
  44784638,	43020437,	43020455,	43021864
))

cdm$ckd_stage_diagnosis <- conceptCohort(
  cdm = cdm,
  ckd_diag_codes, 
...
cdm <- bind(cdm$ckd_stage_meausurement, cdm$ckd_stage_diagnosis, name = "ckd_any")

cdm$ckd_any <- cdm$ckd_any |>
  requireIsFirstEntry()
unionCohorts() |>
  requireIsFirstEntry()
